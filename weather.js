...
const weather = () => {
    ...
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${cityName.value}&appid=bac7c91526d4783920990e14b8cbac2d`)
        ...
        .then(data => {
            ...
            document.getElementById("sunrise").innerHTML = `SunRise: ${sunrise} <br> SunSet: ${sunset}`
...
navigator.geolocation.getCurrentPosition((position) => {
    const lat = position.coords.latitude;
    const lon = position.coords.longitude;
    fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=bac7c91526d4783920990e14b8cbac2d`)
        .then(response => response.json())
        .then(data => {
            console.log(data)
